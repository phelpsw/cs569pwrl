...
public class BoundingSphere {
	...
	public Vector2f rayIntersect(Point3f orig, Vector3f dir) {
		...
		float discriminant = originToCenter.dot(dir);

		if (discriminant < 0.0f) // Ray points away from sphere
			return null;
		...
		float near = discriminant - hitDistance;
		float far = discriminant + hitDistance;
		if (near == 0)
			near = far;
