...
public class BoundingBox {
	...
	Vector3f maxPoint = new Vector3f(Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY);
	...
	public void reset() {
		initialized = false;
		minPoint.set(Float.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE);
		maxPoint.set(Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY);
		
	}
	
	//returns true if bounding boxes overlap
	public boolean intersect(BoundingBox otherBox)
	{
		
		
		Vector3f otherMin = otherBox.getMinPoint();
		Vector3f otherMax = otherBox.getMaxPoint();
		
		return 
		        (minPoint.x < otherMax.x) && (maxPoint.x > otherMin.x) &&
		        (minPoint.y < otherMax.y) && (maxPoint.y > otherMin.y) &&
		        (minPoint.z < otherMax.z) && (maxPoint.z > otherMin.z);
