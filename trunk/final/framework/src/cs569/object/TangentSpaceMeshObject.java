...
public class TangentSpaceMeshObject extends MeshObject {
	...
	public void calculateTangentSpace() {
		...
		for (int i=0; i<numTriangles; i++) {
			...
			if (tangent.length() == 0.0f || binormal.length() == 0.0f) {
				...
				if (tangent.length() == 0.0f) {
					...
					/* If everything goes wrong, at least provide
					   *some* kind of a tangential coordinate system */
					if (tangent.length() == 0)
	...
	private void coordinateSystem(Vector3f a, Vector3f b, Vector3f c) {
	    if (Math.abs(a.x) > Math.abs(a.y)) {
	        float invLen = 1.0f / (float) Math.sqrt(a.x * a.x + a.z * a.z);
	        b.set(-a.z * invLen, 0.0f, a.x * invLen);
	    } else {
	        float invLen = 1.0f / (float) Math.sqrt(a.y * a.y + a.z * a.z);
	        b.set(0.0f, -a.z * invLen, a.y * invLen);
	    }
	    c.cross(a, b);
	}
