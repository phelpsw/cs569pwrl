...
public class Texture {
	...
	public static final Texture getTexture(String identifier) throws UndefinedTextureException {
		...
		if (output == null) {
			output = Texture.create2DTextureFromFile(identifier, identifier, GL.GL_LINEAR, GL.GL_REPEAT, true);
		}
		return output;
	}
	 ...
	 */
	public static Buffer createBufferFromImage(BufferedImage bufferedImage) {
		...
		for (int iy = 0; iy < imageHeight; iy++) {
			for (int ix = 0; ix < imageWidth; ix++) {
				...
				int pixelAlphaValue = 0xff & (pixelValue & 0xff000000) >> 24;
				imageData.put((iy * imageHeight + ix) * 4 + 0, (byte) (((pixelValue & 0x00ff0000) >> 16)
						* pixelAlphaValue / 255));
				imageData.put((iy * imageHeight + ix) * 4 + 1, (byte) (((pixelValue & 0x0000ff00) >> 8)
						* pixelAlphaValue / 255));
				imageData.put((iy * imageHeight + ix) * 4 + 2, (byte) (((pixelValue & 0x000000ff) >> 0)
						* pixelAlphaValue / 255));
				imageData.put((iy * imageHeight + ix) * 4 + 3, (byte) pixelAlphaValue);
			}
		}
		return imageData;
	}
	...
	public static final Texture create2DTextureFromFile(String identifier, String imageFileName, int filterMode, int wrapMode,
		...
		int imageWidth = bufferedImage.getWidth();
		int imageHeight = bufferedImage.getHeight();
		Buffer imageData = createBufferFromImage(bufferedImage);
